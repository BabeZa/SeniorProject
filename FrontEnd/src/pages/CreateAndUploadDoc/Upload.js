...
export default function Upload() {
    ...
    const { modalIsOpenUploadDoc, setModalIsOpenUploadDoc } = useContext(SelectContext);
    ...
    useEffect(() => {
        ...
        if(isSetInit){
            PostgreAPI.post('/document-upload/profilter/'+ profile.id , select1)
            .then((res) => {
    ...
    return (
        <>
        <Modal
            isOpen={modalIsOpenUploadDoc}
            onRequestClose={() => setModalIsOpenUploadDoc(false)}
            style={{
                overlay:{
        ...
        <div className="listall-content">
            ...
            <div className="card-wrapper">
                <div className='card-add card-hover1' onClick={() => setModalIsOpenUploadDoc(true)}> <i className="fas fa-plus "/></div>
                {document.map((item) =>(
                <UploadCard id={item.id} name={item.name} by={item.createbyeng} date={item.createdate} color={item.color} path={item.path} dataID={item.link}/>
                ))}

...
export default function FileUAC() {
  ...
  useEffect(() => {
    ...
    if (profile.successful) {
      PostgreAPI.post("/tqf3/course-myprofile", select1)
        .then((res) => {
          console.log("[Courses.js] Courses get: ", res.data);
          setAllDoc(res.data);
  ...
  const getTermyearSelect = () => {
    PostgreAPI.get("/forselect/termyear/" + localStorage.getItem("i18nextLng"))
      .then((res) => {
        ...
        console.log("[Courses.js] status: " + res.data[0].value);
      })
      .catch((error) => {
        console.error(error);
      });
